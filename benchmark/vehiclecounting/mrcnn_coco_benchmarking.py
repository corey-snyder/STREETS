...
sys.path.append(os.path.join(ROOT_DIR,"samples/coco/") #local path for coco model weights
TRAVEL_CLASSES = ['BG', 'car'] #only classes we are concerned with
MODEL_DIR = os.path.join(ROOT_DIR, "logs") #directory for logs and saved models
...
mode = sys.argv[1]
with open(os.path.join('data/'+mode,'via_region_data.json'),'r') as f:
    val_dict = json.load(f)
img_dir = os.path.join('data',mode)
AE = []
APE = []
ADE = []
ADPE = []
